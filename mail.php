...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $product = $_POST['userProduct'];
    $data = $_POST['userData'];

    // создаем переменную с содержанием письма
    $content = $name . ' оставил заявку на процедуру ' . $product . ' желаемая дата и время ' . $data . '. Его телефон: ' . $phone;

    // Первый параметр - кому отправляем письмо, второй - тема письма, третий - содержание
    $success = mail("admin@whitelotus.com", 'Запрос на процедуру', $content);
